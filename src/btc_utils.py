...
def btc_challenge(
    public_key_bytes, message_hex: str, aggregated_public_nonce: Point
) -> bytes:
...
def calculate_tweaked(share, group_key_pub):
    ...
    Q, tweaked_pubkey = taproot_tweak_pubkey(group_key_pub, b"")
    if not Q.y % 2 == 0:
...
def btc_generate_signature_share(
    tweaked_share, coef, challenge, aggregated_public_nonce, nonce_d, nonce_e, row
):
...
def btc_verify_group_signature(aggregated_signature: Dict) -> bool:
    ...
    return _schnorr_verify(
        bytes.fromhex(aggregated_signature["message"].replace("0x", "")),
        tweaked_pubkey,
        bytes.fromhex(aggregated_signature["public_nonce"]["x"].replace("0x", ""))
        + bytes_from_int(aggregated_signature["signature"]),
    )
